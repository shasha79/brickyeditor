  ...
  export namespace Fields {
    ...
    export class HeritageField extends ImageField {
      ...
      bind() {
        ...
        let $iframe = $('<iframe src="/app/search-widget?id=' + this.instance_id + '" style="width: 100%; height: 100%"></iframe>');
      ...
      setInfoTooltip(item) {
        let $tooltip = this.$field.nextAll(".j-story__photo_credits");
        if ($tooltip) {
        ...
        // Stories title (TODO: extract to separate function|module)
        let $story = this.$field.nextAll(".story-text");
        if ($story) {
          $story.html(this.truncate(item.title, 100));
        }
